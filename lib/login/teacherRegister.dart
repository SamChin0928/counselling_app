...
class TeacherRegister extends StatefulWidget {
  TeacherRegister({Key? key}) : super(key: key);
                        ...
                        Navigator.pushReplacement(
                          ...
                          MaterialPageRoute(
                            builder: (context) => RegisterOTP(
                              teacherName: _nameField.text,
                              teacherPhone: _phoneField.text,
                              teacherEmail: _emailField.text,
                              teacherPassword: _passwordField.text,
                            ),
                          ),
                        );
                      }
...
class _RegisterOTPState extends State<RegisterOTP> {
  ...
  void verify() {
    var res = EmailAuth.validate(
        receiverMail: widget.teacherEmail.toString(), userOTP: _otpField.text);
    if (res) {
      setState(() {
        otpValid = true;
      });
                        ...
                        if (_formKey.currentState!.validate()) {
                          if (otpValid == true) {
                            bool shouldNavigate = await register(
                                widget.teacherEmail.toString(),
                                widget.teacherPassword.toString());
                            await teacherSetup(
